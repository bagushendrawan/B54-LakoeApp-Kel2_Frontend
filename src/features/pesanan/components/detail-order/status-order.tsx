  ...
  return (
    <>
      ...
      <div className="p-3 pl-0">
        <div className={`${color} w-40 rounded-sm text-white flex justify-center items-center p-2 font-semibold`}>
                <p>{props.invoice?.status}</p>
        </div>
        <p className="my-2">
          Pesanan akan dibatalkan bila pembayaran tidak dilakukan sampai {date?.toString()}. Silakan tunggu sampai pembayaran
          terkonfirmasi sebelum mengirimkan barang.
        </p>

        <RiwayatPesanan status={props.invoice?.status} invoice={props.invoice}/>  
      </div>
    </>
  );
}

        ...
        <Dialog>
            <DialogTrigger>
                <Button variant={'outline'} className="rounded-full">
                    {product.variants[0].name} - {product.variants[0].variant_option.length} Varian
                </Button>
            </DialogTrigger>
            <DialogContent className="min-w-fit">
                <DialogHeader>
                    <DialogTitle>Varian <span className="font-bold">{product.name}</span></DialogTitle>
                    <DialogDescription>
                        {product.variants[0].variant_option.map((data) => (
                            <div className="w-full border p-4 mt-4 rounded shadow-md">
                                <div className="w-full flex items-center gap-4">
                                    {/* product image */}
                                    <img
                                        src={data.variant_option_values.img}
                                        alt={product.name}
                                        width={'100rem'}
                                    />
                                    ...
                                    <div className="w-full">
                                        <div className="flex">
                                            {/* variant name */}
                                            <p className="flex flex-1 text-xl text-black font-bold">{data.name}</p>
                                        </div>

                                        <div className="flex flex-col gap-4">
                                            <div className="flex items-center gap-2 mt-2 text-lg">
                                                <p className="font-bold">
                                                    Rp{data.variant_option_values.price}
                                                </p>
                                                <GoDotFill fill="#909090" size={'.8rem'} />
                                                <p className="text-gray-500">Stock : {data.variant_option_values.stock}</p>
                                                <GoDotFill fill="#909090" size={'.8rem'} />
                                                <p className="text-gray-500">SKU : {data.variant_option_values.sku}</p>
                                            </div>

                                            <div className={`flex items-center ${data.variant_option_values.is_active ? 'justify-center' : 'justify-end'}`}>
                                                {product.is_active && (

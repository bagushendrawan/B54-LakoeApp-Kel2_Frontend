...
const ActivateProductDialog: FC<IActivateProductProps> = ({ product }) => {
    ...
    const handleActivate = async () => {
        ...
        const res = await Axios({
            method: 'patch',
            url: `http://localhost:3000/product/activate/a/${product.id}`,
            params: {
                userId: userId
            },
            headers: {
    ...
    const handleNonactivate = async () => {
        ...
        const res = await Axios({
            method: 'patch',
            url: `http://localhost:3000/product/nonactivate/n/a/${product.id}`,
            params: {
                userId: userId
            },
            headers: {
    ...
    return (
        <div>
            ...
            <Dialog open={activateOpen} onOpenChange={setActivateOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            <label className='text-black'>Produk <span className='font-bold'>{product.name}</span> akan diaktifkan</label>
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
            ...
            <Dialog open={nonactivateOpen} onOpenChange={setNonactivateOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle className="text-lg font-bold">Nonaktifkan Produk?</DialogTitle>
                        <DialogDescription>
                            <label className='text-black'>Produk <span className='font-bold'>{product.name}</span> akan dinonaktifkan</label>
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <div className="flex justify-end space-x-2 mt-4">
